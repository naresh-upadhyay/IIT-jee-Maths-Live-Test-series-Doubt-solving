...
public class PostShortNotes extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tab = bundle.getString("tab","activity");
        ...
        Link linkUsername = new Link(Pattern.compile("(@\\w+)"))
                ...
                .setClickListener(new Link.OnClickListener() {
                    ...
                    public void onClick(String text) {
                        // do something
                        Toast.makeText(PostShortNotes.this, "Clicked username: " +text, Toast.LENGTH_SHORT).show();
        ...
        postButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                final String tagstxt = editTextPostTags.getText().toString().trim();
                final String questiontxt = editTextPostQuestion.getText().toString().trim();
                String topicnumtxt = editTextPosttopicnum.getText().toString().trim();
                final String timetxt = editTextPosttime.getText().toString().trim();
                ...
                try{
                    ...
                    favourites= FirebaseDatabase.getInstance().getReference()
                            .child("chapters").child(book).child(chapter).child(tab).child("newdata");
                    favourites.push().child("title").setValue(tagstxt);
                    Query applesQuery = favourites.orderByChild("title").equalTo(tagstxt);
                    applesQuery.addListenerForSingleValueEvent(new ValueEventListener() {
                        @Override
                        public void onDataChange(DataSnapshot dataSnapshot) {
                            for (DataSnapshot appleSnapshot: dataSnapshot.getChildren()) {
//                                appleSnapshot.getRef().child("topicNum").setValue(topicNum);
                                appleSnapshot.getRef().child("title").setValue(tagstxt);
                                appleSnapshot.getRef().child("conceptPdfUrl").setValue(questiontxt);
                                appleSnapshot.getRef().child("time").setValue(timetxt);
                            }
                        }
