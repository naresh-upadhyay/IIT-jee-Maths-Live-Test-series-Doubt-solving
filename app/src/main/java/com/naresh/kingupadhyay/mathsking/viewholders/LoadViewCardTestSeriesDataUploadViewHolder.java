...
public class LoadViewCardTestSeriesDataUploadViewHolder extends RecyclerView.ViewHolder {
    private View view;
    ...
    public LoadViewCardTestSeriesDataUploadViewHolder(View itemView, final int[] marksValueAll) {
        super(itemView);
        view = itemView;
        ...
        final CheckBox checkBoxA = view.findViewById(R.id.checkBoxA);
        final CheckBox checkBoxB = view.findViewById(R.id.checkBoxB);
        final CheckBox checkBoxC = view.findViewById(R.id.checkBoxC);
        final CheckBox checkBoxD = view.findViewById(R.id.checkBoxD);
    ...
    public int marksCalculation(boolean acheck,boolean bcheck,boolean ccheck,boolean dcheck){
        int tempMarks=0;
        if(acheck||bcheck||ccheck||dcheck){
            if(acheck == optionA && bcheck == optionB && ccheck == optionC && dcheck == optionD ){
                return markspos;
            }
            if(acheck){
                if(acheck!=optionA){
                    return marksneg;
                }else tempMarks = tempMarks +1;
            }
            if(bcheck){
                if(bcheck!=optionB){
                    return marksneg;
    ...
    public void setNumericval(final int numericval) {
        ...
        spinner.setSelection(numericval+1);
    }
    ...
    public void setOptions(boolean optionA,boolean optionB,boolean optionC,boolean optionD) {
        ...
        checkBoxA.setChecked(optionA);
        checkBoxB.setChecked(optionB);
        checkBoxC.setChecked(optionC);
        checkBoxD.setChecked(optionD);
